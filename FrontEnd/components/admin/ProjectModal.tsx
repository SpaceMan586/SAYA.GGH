...
export default function ProjectModal({
  ...
  const handleTagChange = (tag: string) => {
    if (newProject.tags.includes(tag)) {
      setNewProject({
        ...newProject,
        tags: newProject.tags.filter((t: string) => t !== tag),
      });
    } else {
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl dark:bg-gray-800 m-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  className="w-full mt-1 p-3 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none appearance-none dark:bg-gray-700 dark:text-white"
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {categories.map((cat: string) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold border transition-colors ${newProject.tags.includes(cat) ? "bg-blue-600 border-blue-600 text-white" : "bg-white dark:bg-gray-700 text-gray-400 hover:border-gray-300"}`}
                    >
                      {cat}
                    </button>
                  ))}
              ...
              <div>
                ...
                <textarea
                  className="w-full mt-1 p-3 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none dark:bg-gray-700 dark:text-white"
                  rows={6}
                  placeholder="Describe the design philosophy..."
        ...
        <div className="p-6 border-t flex gap-3 justify-end bg-gray-50 dark:bg-gray-900 rounded-b-2xl">
          ...
          <button
            onClick={onAdd}
            disabled={uploading}
            className="px-10 py-2.5 bg-gray-900 text-white rounded-lg font-bold hover:bg-gray-800 disabled:opacity-50 transition-all shadow-lg active:scale-95"
          >
            {uploading
              ? "UPLOADING..."
              : editingProjectId
