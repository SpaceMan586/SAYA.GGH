...
export default function LandingBottomBar({
  ...
  return (
    <div className="fixed bottom-0 left-0 z-[100] w-full flex items-center justify-between gap-3 px-4 md:px-12 py-4 pb-4 md:pb-4 bg-white border-t border-gray-100 transition-all duration-300 supports-[padding:0_safe-area-inset-bottom]:pb-[calc(1rem+env(safe-area-inset-bottom))]">
      {/* LEFT SECTION (Visible on Mobile & Desktop) */}
      <div className="flex items-center gap-4 md:gap-8 text-[9px] md:text-[10px] font-semibold tracking-[0.3em] md:tracking-[0.4em] uppercase">
        <LanguageToggle />
      ...
      {/* ICONS SECTION - COMPACT ON MOBILE */}
      <div className="flex md:hidden items-center gap-4 text-base text-black/40">
        {whatsappHref ? (
          <a
            href={whatsappHref}
            target="_blank"
            ...
            aria-label="WhatsApp"
            className="hover:text-black active:scale-95 transition-all"
          >
            <FaWhatsapp />
          </a>
        ) : (
      ...
      <div className="flex items-center justify-end">
        <button
          ...
          className="bg-gray-900 hover:bg-black text-white text-[9px] md:text-[10px] font-semibold tracking-[0.22em] md:tracking-[0.4em] px-4 py-2.5 md:px-10 md:py-3 uppercase transition-all active:scale-95 rounded-sm flex items-center justify-center gap-2 whitespace-nowrap"
        >
          {t("bottom.chatUs")}
        </button>
      </div>
