        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          {loading ? <p>Loading news...</p> : newsList.length === 0 ? <p>No news yet.</p> : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {newsList.map((item) => (
                <div key={item.id} className="bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all group">
                  {item.image_url && (
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm line-clamp-3 mb-6 text-justify">{item.content}</p>
                    <button className="text-sm font-black uppercase tracking-widest border-b-2 border-black dark:border-white pb-1">Read More</button>
                  </div>
                </div>
              ))}
            </div>
