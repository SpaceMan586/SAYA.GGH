...
import { FaUser, FaRobot, FaPaperPlane, FaUserTie, FaCheckCircle, FaTrash, FaArchive } from "react-icons/fa";
...
export default function ChatInbox() {
  ...
  const toggleMode = async (currentStatus: string) => {
    ...
    await supabase.from('chat_sessions').update({ status: newStatus }).eq('id', activeSessionId);
  };

  const handleEndSession = async () => {
    if (!activeSessionId) return;
    if (!confirm("Akhiri sesi chat ini? Pengunjung akan melihat notifikasi selesai.")) return;
    ...
    await supabase.from('chat_messages').insert([{
      session_id: activeSessionId,
      role: 'admin',
      content: "--- Sesi chat diakhiri oleh Admin ---"
    }]);
  };

  const handleDeleteSession = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation(); // Mencegah klik parent (pilih sesi)
    if (!confirm("Hapus permanen sesi chat ini? Data tidak bisa dikembalikan.")) return;
          ...
          {sessions.map((session) => (
            <div
              ...
              className={`group p-4 border-b border-gray-100 cursor-pointer transition-colors hover:bg-white relative ${
                activeSessionId === session.id ? "bg-white border-l-4 border-l-black" : ""
              } ${session.status === 'closed' ? "opacity-60 bg-gray-100" : ""}`}
            >
              <div className="flex justify-between items-center mb-1 pr-6">
                <span className="font-bold text-xs uppercase tracking-wider text-gray-500">Visitor {session.id.slice(0, 4)}</span>
                <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase border ${
                  session.status === 'human' ? 'bg-blue-50 text-blue-600 border-blue-200' : 
                  session.status === 'closed' ? 'bg-gray-200 text-gray-500 border-gray-300' :
              ...
              <div className="flex items-center gap-3">
                <div>
                  <h3 className="font-bold flex items-center gap-2">
                    Live Chat 
                    {activeSessionData?.status === 'closed' && <span className="text-xs font-normal text-red-500">(Selesai)</span>}
                  </h3>
              ...
              <div className="flex gap-2">
                {activeSessionData?.status !== 'closed' && (
                  <>
                    ...
                    >
                      {activeSessionData?.status === 'human' ? <><FaRobot /> AI Mode</> : <><FaUserTie /> Take Over</>}
                    </button>
                ...
                {activeSessionData?.status === 'closed' && (
                  <button onClick={(e) => handleDeleteSession(e, activeSessionId)} className="text-red-500 hover:text-red-700 p-2 text-sm"><FaTrash /> Hapus</button>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
              {messages.map((msg) => (
                ...
                >
                  ...
                  >
                    {msg.role === 'bot' && !msg.content.includes("---") && <strong className="block text-[10px] uppercase mb-1 not-italic">AI Assistant</strong>}
                    {msg.content}
                  </div>
                </div>
              ))}
            ...
            {activeSessionData?.status !== 'closed' ? (
              <form onSubmit={handleSend} className="p-4 border-t border-gray-200 bg-white flex gap-2">
                <input
                  ...
                  placeholder={activeSessionData?.status === 'ai' ? "Mode AI Aktif. Switch ke Human untuk membalas..." : "Ketik balasan..."}
                ...
                <button
                  type="submit"
                  disabled={activeSessionData?.status === 'ai' || !inputText.trim()}
