...
export default function TrainingTab() {
  ...
  const handleAdd = async () => {
    if (!newKnowledge.topic || !newKnowledge.content) return alert("Mohon isi topik dan konten.");

    const { error } = await supabase.from("ai_knowledge").insert([newKnowledge]);
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight">AI Training Center</h2>
          <p className="text-gray-500 mt-1 text-sm">Ajari AI Anda fakta-fakta tentang studio, harga, atau proyek spesifik.</p>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-gray-400 mb-1">Topik / Judul</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-gray-400 mb-1">Isi Pengetahuan (Fakta)</label>
                <textarea
                  rows={6}
        ...
        <div className="lg:col-span-2 space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
              ...
              <p className="text-gray-500 font-medium">Belum ada pengetahuan.</p>
              <p className="text-xs text-gray-400">Mulailah dengan menambahkan info dasar studio.</p>
            </div>
          ) : (
            knowledgeList.map((item) => (
              <div key={item.id} className="group bg-white p-5 rounded-xl border border-gray-100 hover:shadow-md transition-all flex justify-between items-start gap-4">
